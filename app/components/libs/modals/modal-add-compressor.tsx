...
export default function AddCompressorModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      openWarning(t('컴프레셔 추가 실패'), e?.message ?? t('컴프레셔 추가 중 오류가 발생했습니다.'));
    }
  };
  ...
  const modalUi = (
    <>
      ...
      <S.ModalWrap role="dialog" aria-modal="true" aria-labelledby="add-compressor-title">
        ...
        <S.ModalBody>
          <S.BodyTitle id="add-compressor-title">{t('컴프레셔 장비 추가')}</S.BodyTitle>
          <S.BodyDescription>{t('추가할 컴프레셔 장비 정보를 입력해주세요.')}</S.BodyDescription>
        ...
        <S.ModalFooter>
          <S.SubmitButton type="button" onClick={handleSubmit}>{t('추가하기')}</S.SubmitButton>
        </S.ModalFooter>
