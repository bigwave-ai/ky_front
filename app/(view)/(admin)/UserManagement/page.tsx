...
// 좌측 고객사 테이블에서 정렬 가능한 컬럼 키
type CompanySortKey = 'name' | 'businessType' | 'handlingItem' | 'managerPhone' | 'managerEmail'
// 정렬 방향(오름차순/내림차순)
...
export default function UserManagementPage() {
  ...
  const [sortKey, setSortKey] = useState<CompanySortKey | null>(null) // 좌측 테이블 정렬 컬럼(미지정 시 원본 순서)
  const [sortDir, setSortDir] = useState<SortDirection>('asc') // 좌측 테이블 정렬 방향
  ...
  const sortedCompanies = useMemo(() => {
    // 정렬 컬럼이 지정된 경우에만 오름/내림차순 정렬, 미지정 시 원본 순서 유지
  ...
  const updateModalInitialData = useMemo<UpdateCompanyInitialDataType | null>(() => {
    ...
    // 빈 값 표시용 '-' 는 폼에서 빈 문자열로 정규화(컴프레서 모달과 동일) — '-'가 형식검증에 걸리지 않게
    const noDash = (v?: string) => (v && v !== '-' ? v : '')
    return {
      customerId: selectedCompany.id,
      accountId: noDash(selectedCompany.accountId),
      companyName: noDash(selectedCompany.name),
      businessType: noDash(selectedCompany.businessType),
      handlingItem: noDash(selectedCompany.handlingItem),
      managerName: noDash(selectedCompany.managerName),
      managerPhone: noDash(selectedCompany.managerPhone),
      managerEmail: noDash(selectedCompany.managerEmail),
    }
  }, [selectedCompany])
  ...
  // 좌측 고객사 테이블 컬럼 정의(헤더 라벨 + 정렬 키)
  const companyColumns: { key: CompanySortKey; label: string }[] = [
    { key: 'name', label: t('고객사') },
    { key: 'businessType', label: t('업종') },
    { key: 'handlingItem', label: t('취급물품') },
    { key: 'managerPhone', label: t('담당자 연락처') },
    { key: 'managerEmail', label: t('담당자 메일') },
  ]
  ...
  // 좌측 고객사 테이블을 지정 컬럼/방향으로 정렬(같은 방향 재클릭 시 정렬 해제)하는 함수
  const handleSortCompanies = (key: CompanySortKey, direction: SortDirection) => {
    setCurrentPage(1)
    if (sortKey === key && sortDir === direction) {
      setSortKey(null)
      setSortDir('asc')
      return
    }
  ...
  const fetchCustomers = useCallback(async (preferredId?: string) => {
    ...
    try {
      ...
      if (!response.ok || !json.success) {
        throw new Error(json.message ?? t('고객사 조회 실패'))
      }
      ...
      console.error(error)
      setApiError(error?.message ?? t('고객사 목록을 불러오지 못했습니다.'))
      setCompanies([])
    ...
    try {
      ...
      if (!response.ok || !json.success) {
        const message =
          response.status === 409
            ? t('이미 사용중인 아이디입니다.')
            : json.message ?? t('고객사 정보 변경에 실패했습니다.')
        throw new Error(message)
      }
  ...
  const handleDeleteCompany = async (customerId: string) => {
    ...
    try {
      ...
      if (!response.ok || !json.success) {
        throw new Error(json.message ?? t('고객사 삭제에 실패했습니다.'))
      }
  ...
  const fetchCompressorMeta = useCallback(async () => {
    ...
    if (!response.ok || !json.success) {
      throw new Error(json.message ?? t('장비 코드 목록 조회 실패'))
    }
  ...
  const handleAddCompressor = async (form: AddCompressorFormType) => {
    if (!selectedCompany) throw new Error(t('고객사를 먼저 선택해주세요.'))
    ...
    try {
      ...
      if (!response.ok || !json.success) {
        throw new Error(json.message ?? t('컴프레셔 장비 추가 실패'))
      }
  ...
  const handleUpdateCompressor = async (form: UpdateCompressorFormType) => {
    ...
    try {
      ...
      if (!response.ok || !json.success) {
        throw new Error(json.message ?? t('컴프레셔 장비 수정 실패'))
      }
  ...
  const handleDeleteCompressor = async (deviceId: string) => {
    ...
    try {
      ...
      if (!response.ok || !json.success) {
        throw new Error(json.message ?? t('컴프레셔 장비 삭제 실패'))
      }
  ...
  useEffect(() => {
    ...
    fetchCompressorMeta().catch((e) => {
      // 초기 컴프레셔 메타 조회 실패 처리
      console.error(e)
      setApiError(e?.message ?? t('장비 코드 목록을 불러오지 못했습니다.'))
    })
  }, [fetchCustomers, fetchCompressorMeta, t])

  useEffect(() => {
    // 검색/정렬 변경 시 선택 고객사 유효성 보정(선택이 사라지면 정렬 순서상 첫 행을 기본 선택)
    if (!sortedCompanies.length) {
      setSelectedCompanyId('')
      return
    }

    const exists = sortedCompanies.some((company) => company.id === selectedCompanyId)
    if (!exists) {
  ...
  return (
    <div className={umc.user_root}>
      <header className={umc.user_pageHead}>
        <h1>{t('고객사 및 컴프레셔 관리')}</h1>
        <p>{t('고객사와 해당 고객사의 컴프레셔 정보를 관리할 수 있는 페이지입니다.')}</p>
      </header>

      <section className={umc.user_statCard}>
        <div className={umc.user_cardTitleWrap}>
          <h2>{t('고객사 및 컴프레셔 통계')}</h2>
          <p>{t('현재, 고객사의 수와 전체 컴프레셔 등록 수 입니다.')}</p>
        </div>
        ...
        <article ref={leftPanelRef} className={`${umc.user_panel} ${umc.user_leftPanel}`}>
          <div className={umc.user_panelHead}>
            <div className={umc.user_panelTitleWrap}>
              <h3>{t('고객사 관리')}</h3>
              <p>{t('고객사에 대해 추가/삭제/수정할 수 있습니다.')}</p>
            </div>
          ...
          <div className={umc.user_searchRow}>
            <input
              ...
              placeholder={t('고객사, 업종, 취급물품, 연락처, 메일 등 텍스트를 검색해주세요.')}
            />
            <button type="button" className={umc.user_searchBtn} aria-label={t('고객사 검색')}>
              ...
              <thead>
                <tr>
                  {companyColumns.map((col) => {
                    ...
                    return (
                      <th key={col.key}>
                        <div className={umc.user_thInner}>
                          <span>{col.label}</span>
                          <span className={umc.user_sortBtns}>
                            <button
                              type="button"
                              className={`${umc.user_sortBtn} ${isAsc ? umc.user_sortBtnActive : ''}`}
                              onClick={() => handleSortCompanies(col.key, 'asc')}
                              aria-label={`${col.label} ${t('오름차순 정렬')}`}
                              aria-pressed={isAsc}
                            >
                              <i
                                className={`${umc.user_sortCaret} ${umc.user_sortCaretUp}`}
                                aria-hidden="true"
                              />
                            </button>
                            <button
                              type="button"
                              className={`${umc.user_sortBtn} ${isDesc ? umc.user_sortBtnActive : ''}`}
                              onClick={() => handleSortCompanies(col.key, 'desc')}
                              aria-label={`${col.label} ${t('내림차순 정렬')}`}
                              aria-pressed={isDesc}
                            >
                              <i
                                className={`${umc.user_sortCaret} ${umc.user_sortCaretDown}`}
                                aria-hidden="true"
                              />
                            </button>
                          </span>
        ...
        >
          ...
          <div className={umc.user_searchRow}>
            <input
              ...
              className={umc.user_searchInput}
              placeholder={t('컴프레셔에 대한 정보를 검색해주세요.')}
            />
            <button type="button" className={umc.user_searchBtn} aria-label={t('컴프레셔 검색')}>
              <i className={imag.search_grey_icon} aria-hidden="true" />
            </button>
          </div>
            ...
            <section className={umc.user_infoCard}>
              <div className={umc.user_infoHead}>
                <div className={umc.user_infoTitleWrap}>
                  <h4>{t('고객 정보')}</h4>
                  <p>{t('고객 정보 중 수정 가능한 정보를 확인하시고 변경사항이 있을 시 변경해주세요.')}</p>
                </div>
              ...
              <div className={umc.user_infoRows}>
                <div className={umc.user_infoRow}>
                  <div className={umc.user_labelCell}>{t('아이디')}</div>
                  <div className={umc.user_valueCell}>{selectedCompany?.accountId ?? '-'}</div>
                </div>
                <div className={umc.user_infoRow}>
                  <div className={umc.user_labelCell}>{t('비밀번호')}</div>
                  <div className={umc.user_valueCell}>{selectedCompany?.passwordMask ?? '-'}</div>
                </div>
                <div className={umc.user_infoRow}>
                  <div className={umc.user_labelCell}>{t('담당자명')}</div>
            ...
            {filteredCompressors.length ? (
              filteredCompressors.map((cp, index) => (
                <section key={cp.id} className={umc.user_infoCard}>
                  ...
                  <div className={umc.user_infoRows}>
                    <div className={umc.user_infoRow}>
                      <div className={umc.user_labelCell}>{t('장비 명')}</div>
                      <div className={umc.user_valueCell}>{cp.deviceName}</div>
                    </div>
                    <div className={umc.user_infoRow}>
                      <div className={umc.user_labelCell}>{t('시리얼 번호')}</div>
                      <div className={umc.user_valueCell}>{cp.serialNumber}</div>
                    </div>
                    <div className={umc.user_infoRow}>
                      <div className={umc.user_labelCell}>{t('장비 타입')}</div>
                      <div className={umc.user_valueCell}>{cp.equipmentType}</div>
                    </div>
                    <div className={umc.user_infoRow}>
                      <div className={umc.user_labelCell}>{t('데이터 타입')}</div>
                      <div className={umc.user_valueCell}>{cp.dataType}</div>
                    </div>
                    <div className={umc.user_infoRow}>
                      <div className={umc.user_labelCell}>{t('장비 마력')}</div>
              ...
              <div className={umc.user_emptyDetail}>{t('조건에 맞는 컴프레셔가 없습니다.')}</div>
            )}
          </div>
        </article>
      </section>
